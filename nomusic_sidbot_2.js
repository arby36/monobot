...
if(input.startsWith("&NEWVOTE")) {
    ...
    //PM Spam (You can fgure this one out xD)
    ...
    //Gets channel logs (Buggy)
    if (input === "&CHANNELLOGS") {
        try {
            number_raw = input.split(" ");
            number_raw.shift();
            number = number_raw.join(" ");
            if (number <= 50) {
                bot.getChannelLogs(message.channel.id, number, function (error, messages) {
                    bot.sendMessage(message, "Last " + number + " messages: " + messages.join(", \n "));
                });
                console.log("Channel Logs were requested.");
            }
            else {
                bot.sendMessage(message, "The number of messages requested is too high!");
            }
        }
        catch (error) {
            console.log("An error occured while requesting Channel logs: " + error);
        }
    ...
    if (input === "&AFKOFF") {
        if (afk.indexOf(message.mentions[0]) != -1) {
            bot.sendMessage(message, "Un-AFKed! xD");
            var aIndex = afk.indexOf();
            afk.splice(aIndex, 1);
        }
        else {
